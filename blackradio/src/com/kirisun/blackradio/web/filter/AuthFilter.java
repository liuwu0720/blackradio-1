package com.kirisun.blackradio.web.filter;
...
/**
 * 系统过滤器 此过滤器通过统一对提交的访问进行身份校验，过滤非法访问，所有访问必须先进行用户名密码的登录验证后方可进行。
 * 
...
public class AuthFilter implements Filter {
	...
	public void doFilter(ServletRequest servletRequest,
		...
		if (allow(path, uri, exclude)) {
			...
			return;
		} else if (session.getAttribute(SystemCommon.SESSION_USER) != null) {
			chain.doFilter(request, response);
			return;
		} else {
			AjaxUtil.rendResult(response, SystemCommon.CODE_ERROR2, "会话超时，重新登录");
		}
